  ...
  <form>
    <span v-if="showButtons">
      ...
      <div class="action_buttons">
        ...
        <div class="mt-8 lang">
          Google offered in: <router-link to="/">Kinyarwanda</router-link>
        </div>
      </div>
    </span>
    <span v-else>
      <div class="search withOutButtons">
        <v-text-field
          v-model="searchTerm"
          append-icon="mdi-microphone"
          prepend-inner-icon="mdi-magnify"
    ...
    async search(event) {
      ...
      try {
        ...
        let response = await this.axios.get(
          `https://www.googleapis.com/customsearch/v1?key=${this.API_KEY}&cx=${this.CONTEXT_KEY}&q=${this.searchTerm}`
        );
